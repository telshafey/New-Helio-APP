...
import { PhoneIcon, WhatsAppIcon, ShareIcon, HeartIcon, HeartIconSolid, ClockIcon, MapPinIcon, StarIcon, HandThumbUpIcon } from '../components/Icons';
...
const ServiceDetailScreen = () => {
  ...
  const { services, handleToggleFavorite } = useServices();
  ...
  const handleShare = async () => {
      try {
          await Share.share({
              message: `تحقق من هذه الخدمة: ${service.name} في تطبيق هيليو.`,
      ...
      <View style={styles.contentContainer}>
        ...
        <View style={styles.actionsContainer}>
            ...
            {isPublicAuthenticated && (
                <Pressable style={[styles.actionButton, {backgroundColor: '#ef4444'}]} onPress={() => handleToggleFavorite(service.id)}>
                    {service.isFavorite ? <HeartIconSolid color="#fff" width={24} height={24} /> : <HeartIcon color="#fff" width={24} height={24} />}
                </Pressable>
            )}
        </View>
            ...
            <View style={styles.section}>
                 {service.workingHours && (
                    <View style={styles.infoRow}>
                        <ClockIcon color="#64748b" width={20} height={20} />
                        <Text style={styles.infoText}>{service.workingHours}</Text>
                    </View>
                 )}
                 {service.locationUrl && (
                    <View style={styles.infoRow}>
                        <MapPinIcon color="#64748b" width={20} height={20} />
                        <Text style={[styles.infoText, styles.linkText]} onPress={() => Linking.openURL(service.locationUrl!)}>عرض على الخريطة</Text>
                    </View>
                 )}
            </View>
        )}
                    ...
                    <View key={review.id} style={styles.reviewCard}>
                        ...
                        {review.adminReply && (
                            <View style={styles.adminReply}>
                                <Text style={styles.adminReplyTitle}>رد الإدارة:</Text>
