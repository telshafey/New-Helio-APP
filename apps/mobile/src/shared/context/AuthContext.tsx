...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const [currentUser, setCurrentUser] = useState<AdminUser | null>(() => {
        const storedUser = storage.getSessionItem('currentUser');
        return storedUser ? JSON.parse(storedUser) : null;
    });

    const [currentPublicUser, setCurrentPublicUser] = useState<AppUser | null>(() => {
        const storedUser = storage.getSessionItem('currentPublicUser');
        return storedUser ? JSON.parse(storedUser) : null;
    });
    ...
    const login = useCallback((email: string, password?: string): boolean => {
        ...
        if (admin) {
            storage.setSessionItem('currentUser', JSON.stringify(admin));
    ...
    const logout = useCallback(() => {
        storage.removeSessionItem('currentUser');
        setCurrentUser(null);
    ...
    const publicLogin = useCallback((email: string, password?: string): boolean => {
        ...
        if (user) {
            ...
            storage.setSessionItem('currentPublicUser', JSON.stringify(user));
    ...
    const publicLogout = useCallback(() => {
        storage.removeSessionItem('currentPublicUser');
    ...
    const register = useCallback((newUserData: Omit<AppUser, 'id' | 'joinDate' | 'avatar' | 'status' | 'role'>): boolean => {
        ...
        storage.setSessionItem('currentPublicUser', JSON.stringify(newUser));
    ...
    const updateProfile = useCallback((updatedUser: Omit<AppUser, 'joinDate' | 'status' | 'password' | 'role'>) => {
        ...
        if (currentPublicUser) {
            storage.setSessionItem('currentPublicUser', JSON.stringify(finalUser));
        }
